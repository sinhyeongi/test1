...
public class Controller {
	...
	private void NewStudent() {
		String id = Util.getString("id를 입력해주세요");
		if(studao.CheckStuId(id)) {
			System.err.println("중복된 id입니다.");
			return;
		}
		String name = Util.getString("이름을 입력해주세요");
	...
	private void DeleteStudent() {
		...
		String id = Util.getString("[학생삭제]아이디를 입력하세요");
	...
	private void NewSubject() {
		int hackbun = Util.getInt("학번을 입력하세요");
		if(hackbun == -1) return;
		if(studao.Studenthackbun(hackbun) == -1) {
			System.err.println("[과목추가]잘못된 학번 입니다.");
			return;
		}
		String name = Util.getString("과목 이름을 입력해주세요");
	...
	//메인실행부분
	public void run() {
		RoadFile(Util.getInstance().RoadText());
		while(true) {
			mainMenu();
			int i = Util.getInt("메뉴 선택");
			if(i == 0 ) {
			...
			if( i == 1) {
				...
				String name = Util.getString("찾을 과목 이름 입력");
				...
				Util.getInstance().SaveText(studao.SaveData(), subdao.SaveData());
			}
			else if(i == 8) {
				RoadFile(Util.getInstance().RoadText());
			}
